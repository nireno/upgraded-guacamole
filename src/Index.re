...
type machineState =
  | Loading
  // | MainMenuState(mainMenuState, ServerPublicState.t)
...
module App = {
  ...
  let make = () => {
    ...
    let (clientSettings, updateClientSettings) =
      React.useState(() => LocalStorage.getClientSettings());
    let (canJoinPublicGame, updateCanJoinPublicGame) = React.useState(() => true);
    let (machineState, updateMachineState) = React.useState(() => Loading);
          ...
          | Belt.Result.Ok(serverState) =>
            let update = machineState =>
              ...
              | ActiveGameState(gameState) =>
                let nextGameState =
                  Client.GameReducer.reducer(gameState, MatchServerState(serverState));
    ...
    <div ref={ReactDOMRe.Ref.domRef(appRef)}> "foo"->React.string </div>;
    ...
    //     //       />
    //     //     <div className={j|the-rest relative flex-grow flex $bgBoard justify-between|j}
    ...
    //     //         />
    //     //       </div>
    //     //       <div className="mid-partners relative self-stretch w-1/3 flex flex-col justify-between">
    //     //         {
...
module SocketConsumer = {
  [@react.component]
  let make = () => {
    let socket = React.useContext(SocketProvider.socketContext);
    React.useEffect1(
      () => {
        ClientSocket.T.on(socket, x =>
          switch (x) {
          | Reset => Js.log("got reset")
          | _ => Js.log("got something else")
          }
        );
        None;
      },
...
module SocketApp = {
  ...
  let make = () => {
    ...
    <div className="a4s_app">
      <SocketProvider value=socket> <SocketConsumer /> </SocketProvider>
    </div>;
  };
};

ReactDOMRe.renderToElementWithClassName(<SocketApp />, "app");
