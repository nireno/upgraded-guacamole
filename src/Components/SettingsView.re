...
let make = (~onSave, ~settings) => {
  ...
  <form className="bg-white shadow-md border border-solid border-gray-200 rounded px-8 pt-6 pb-8 mb-4">
    ...
    <div className="mb-4 flex flex-row ">
      <input  checked={substitution ? true : false} type_="radio" name="substitute" id="substitute-yes" onClick={_ => updateSubstitution(_prev => true)}/>
    ...
    <div className="mb-4 flex flex-row ">
      <input type_="radio" name="substitute" id="substitute-no" checked={substitution ? false : true} onClick={_ => updateSubstitution(_prev => false)}/>
