...
module Filter = {
  ...
  let simplifyPhase: phase => simplePhase = gamePhase =>
    switch gamePhase {
...
let isFindPlayersPhase = x =>
  switch x {
  | FindPlayersPhase(_) => true
  | _ => false
  }

let isFindSubsPhase = x =>
  switch x {
